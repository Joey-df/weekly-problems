...
public class Code05_Colors {
	...
	public static boolean ok(Node a, Node b, HashMap<Node, Node> map) {
		...
		Node cur = a;
		while (cur != null) { //一路向上撺
			aPath.add(cur);
			cur = map.get(cur);
		}
		Node lowest = b; //lowest：寻找到的a b的最低公共祖先
		while (!aPath.contains(lowest)) {
			lowest = map.get(lowest);
		}
		int colors = 1 << lowest.color;
		cur = a;
		while (cur != lowest) { //从a一路向上撺
			colors |= (1 << cur.color);
			cur = map.get(cur);
		}
		cur = b;
		while (cur != lowest) { //从b一路向上撺
			colors |= (1 << cur.color);
			cur = map.get(cur);
		}
		return colors == 15; //表示 0 1 2 3 都存在（因为都左移了1位）
	}
	...
	public static Info process2(Node h) {
		...
		if (!h.nexts.isEmpty()) {
			...
			// 头节点出发，全颜色搞定，100个，200（算上以头节点结尾的100个）
