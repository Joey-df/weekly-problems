...
public class Code01_StronglyConnectedComponents {
	...
	public static class StronglyConnectedComponents {
		...
		public int[] low;
		public int cnt;
		public int[] scc; //每个节点隶属集团（数组下标从1开始，数组下标表示节点号，对应的值为隶属集团号）
		public int sccn; //集团号
		...
		private void tarjan(int p) {
			low[p] = dfn[p] = ++cnt;
			stack[stackSize++] = p; //栈中放的是原始节点本身
			for (int q : nexts.get(p)) {
				// q 当前p的每一个孩子
				if (dfn[q] == 0) { //等于0说明当前q这个孩子还没遍历过，没遍历过就去遍历
					tarjan(q);
				}
				// q 肯定遍历过  1) 遍历过，结算了！2）遍历过，没结算
				if (scc[q] == 0) { // scc[q]!=0 一定表示q已经属于某个集团了！不能用来更新
